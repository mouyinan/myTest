...
public class OrderAction extends ActionSupport{
	...
	public String addOrder() throws Exception{
		...
		customer = customerDao.queryCustomerInfo(customer.getName()).get(0);//如果有重复的话，就只会查找到第一个出现的那个用户名，根据用户名去查询这个用户。
		Order ord =new Order();
		...
		ord.setTotal(foodDao.GetFoodById(food.getFoodid()).getVipprice()*1);}
		if(customer.getRole()==2){
			ord.setTotal(foodDao.GetFoodById(food.getFoodid()).getPrice()*1);}
